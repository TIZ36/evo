...
`evo-memory` 是一个独立的 Agent 记忆与进化组件。它从回合经验中蒸馏记忆和可复用技能，并向不同 Agent 运行时提供检索、注入、整合和删除能力。
...
默认路径由运行时提供合理值，不写死为 Paper 的路径。例如，默认数据目录可以是平台数据目录下的 `evo-memory` 子目录。用户可以通过配置或环境变量一次性改写数据目录、数据库文件和物化目标。
...
先保证 DeepSeek Harness 中的记忆闭环可用，再逐步增加 Paper、Claude、Codex adapter。任何宿主适配都不能反向污染 `evo-memory` core 的协议。
