...
mkdir -p "$CLAUDE_DIR"
node --input-type=module - "$SETTINGS" "$HOOK_ENTRY" "$MODE" "$CLAUDE_DIR" <<'NODE'
import { copyFileSync, existsSync, readdirSync, readFileSync, writeFileSync } from 'node:fs'
import { join } from 'node:path'

const [, , settingsPath, hookEntry, mode, claudeDir] = process.argv
...
const pluginPath = findClaudePlugin()
if (pluginPath && mode === 'install') {
  ...
  console.error(`       /plugin uninstall evo`)
...
if (mode === 'install') {
  ...
  if (pluginPath) {
    ...
    console.log(`     To fully remove evo, also run: /plugin uninstall evo`)
  }
}
NODE
...
if (!existsSync(projectSettings)) process.exit(0)
